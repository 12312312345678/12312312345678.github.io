...
import 'package:religous_lifetype_inventory/pit_pages/pit_survey.dart';

class PITHome extends StatefulWidget {
  const PITHome({super.key, required this.storage});
  final String storage;

  @override
  State<PITHome> createState() => PITHomePage();
}

class PITHomePage extends State<PITHome> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: const Text("PIT, Personal Text Inventory"),
          ),
          body: Center(
            child: Column(
              children: [
                ...
                const Padding(
                  padding: EdgeInsets.all(8.0),
                  child: Text("이 검사는 신앙의 유형과 특징을 구분해줍니다."),
                ),
                const Padding(
                  padding: EdgeInsets.all(8.0),
                  child: Text(
                ...
                Padding(
                  ...
                  child: ElevatedButton(
                      onPressed: () {
                        ...
                        context.read<PITAnswers>().init();
                        Navigator.pop(context);
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => const PITSurveyWidget()));
                      },
