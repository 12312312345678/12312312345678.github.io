...
class _ERLISurveyState extends State<ERLISurvey> {
  ...
  List<ERLISurveyPage> maniGenerator() {
    ...
    for (ERLIType type in ERLIType.basisTypes) {
      chapter1.addAll(autoGenerator(type));
    }
    List<ERLISurveyPage> chapter2 = [];
    for (ERLIType type in ERLIType.basisAnties) {
      chapter2.addAll(autoGenerator(type));
    }
    List<ERLISurveyPage> chapter3 = [];
    for (ERLIType type in ERLIType.doubleTypes) {
      chapter3.addAll(autoGenerator(type));
    }
    chapter1.shuffle();
    chapter2.shuffle();
    chapter3.shuffle();
    chapter1.addAll(chapter2);
    chapter1.addAll(chapter3);
    return chapter1;
  }
}
