...
class _ERLIResultState extends State<ERLIResult> {
  ...
  void initState() {
    ...
    _sheet = ERLISheet(
        ...
        inventory: inv);
    _sheet.enroll(context.read<PreferencesRLITestApp>().storage, additional: {
  ...
  Widget build(BuildContext context) {
    ...
    for (var factor in scoring.keys) {
      ...
      outputs.add(Padding(
        ...
        child: Text(
            "요인: ${ERLIType.parsing(factor).display}, 원점수: ${scoring[factor]![0]},${scoring[factor]![1]},${scoring[factor]![2]} 괴리율: $error 오차율: $diap"),
      ));
    }
    return Scaffold(
      appBar: AppBar(
