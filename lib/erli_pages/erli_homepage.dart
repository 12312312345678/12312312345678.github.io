...
import 'package:religous_lifetype_inventory/erli_pages/erli_survey.dart';

class ERLIHome extends StatefulWidget {
  const ERLIHome({super.key, required this.storage});
  final String storage;

  @override
  State<ERLIHome> createState() => ERLIHomePage();
}

class ERLIHomePage extends State<ERLIHome> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("ERLI, 심층화된 신앙 특징 검사에 오신 것을 환영합니다."),
      ),
      body: Center(
        child: Column(
          children: [
            const Padding(
              ...
              child: Text("개인 신앙 특징 검사 PIT에 오신 것을 환영합니다."),
            ),
            const Padding(
              padding: EdgeInsets.all(8.0),
              child: Text("이 검사는 신앙의 유형과 특징을 세분화해서 측정합니다."),
            ),
            const Padding(
              padding: EdgeInsets.all(8.0),
              child: Text(
            ...
            Padding(
              ...
              child: ElevatedButton(
                  onPressed: () {
                    ...
                    context.read<Answers2>().initAnswer(3);
                    Navigator.pop(context);
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => const ERLISurvey()));
                  },
